...
#ifdef OPENDNS
size_t resize_pseudo_packet(HEADER *header, size_t plen, unsigned char *pheader, size_t hlen)
{
  unsigned char *ansp = skip_questions(header, plen);
  //albert
//---
  struct pseudoheader_t *pseudo_header= (struct pseudoheader_t *)pheader;
#ifdef OPENDNS_DBG   
  char *dump_packet;
  int i;
#endif  
  ...
  if (pheader && ntohs(header->arcount) == 0)
    {
      ...
      *(unsigned char *)(ansp) = pseudo_header->name;
      ansp += 1;

      PUTSHORT(pseudo_header->type, ansp);
      PUTSHORT(pseudo_header->class, ansp);
      PUTLONG(pseudo_header->ttl, ansp);  
      PUTSHORT(pseudo_header->rdlen, ansp);
      PUTSHORT(pseudo_header->rr.op_code, ansp);
      PUTSHORT(pseudo_header->rr.data_len, ansp);
      memcpy(ansp, pseudo_header->rr.data, pseudo_header->rr.data_len);
      header->arcount = htons(1);
      ansp += pseudo_header->rr.data_len;     
    } 
#ifdef OPENDNS_DBG    
    dump_packet = ansp - pseudo_header->rr.data_len - 15;
    for(i = 0; i < (pseudo_header->rr.data_len + 15); i++)
    {
       DEBUG_P("0x%02x ", *(unsigned char*)(dump_packet +i));
       if (i !=0 && i%8 ==0)
        DEBUG_P("\n");	
    }
#endif     

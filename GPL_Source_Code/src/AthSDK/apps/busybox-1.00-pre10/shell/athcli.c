 ...
 *
 * $Id: athcli.c,v 1.1.1.1 2007/08/30 10:28:26 roy Exp $
 */
...
enum {
	...
	CLI_PRIV_RADIO_PARAM_SHORTPREAMBLE,
...
static int cli_set_power(struct cli_param *param, int param_index, int power);
...
/* WSC interface */
static int cli_get_wsc(struct cli_param *param, int param_index);
static int cli_set_wsc(struct cli_param *param, int param_index, int enable);
...
static struct cli_param cli_param_list[] = {
    ...
    { "power", CLI_F_STOPIF | CLI_F_RADIO | CLI_FORMAT_INTEGER,
    ...
    { "wsc", CLI_FORMAT_BOOLEAN,
      (cli_handler)cli_get_wsc, (cli_handler)cli_set_wsc }, /* CLI_PARAM_WSC */
};
...
static struct cli_param cli_priv_radio_param_list[] = {
    ...
    { "shortpreamble", CLI_F_STOPIF | CLI_F_RADIO | CLI_FORMAT_BOOLEAN
                | IEEE80211_PARAM_SHORTPREAMBLE,
      (cli_handler)cli_get_param, (cli_handler)cli_set_param }, /* CLI_PRIV_PARAM_SHORTPREAMBLE */
};
...
static int cli_set_default(void)
{
	...
	radio->priv_params[CLI_PRIV_RADIO_PARAM_SHORTPREAMBLE] = 1; // Enable Short preamble 
...
static int cli_set_wmode_value(int wmode)
{
	...
	if(current_bss->iface)
		sprintf(cmdbuf, "iwpriv %s mode %s\n",
		        current_bss->iface, cli_wmode_list[wmode].iw_name);
	else
		sprintf(cmdbuf, "iwpriv %s mode %s\n",
...
static int cli_set_channel(struct cli_param *param, int param_index, char *channel_txt)
{
	...
	if(current_bss->iface)
		sprintf(cmdbuf, "iwconfig %s channel %s", current_bss->iface, channel_txt);
	else
...
static int cli_set_power(struct cli_param *param, int param_index, int power)
{
    ...
    sprintf(cmdbuf, "iwconfig %s txpower %d", current_radio->iface, power);
    rc = cli_exec_cmd(cmdbuf);
...
static int cli_set_wsc(struct cli_param *param, int param_index, int enable)
{
        apcfg->wsc_enabled = enable;
        return 0;
}

static int cli_get_wsc(struct cli_param *param, int param_index)
{
        return apcfg->wsc_enabled;
}
...
static int cli_set_js(struct cli_param *param, int param_index, int enable)
{
	...
	if (!enable) {
		/* Invalidate the old jumpstart password */
		current_bss->js_passwd[0] = 0;
	}
	return 0;
}
...
static int cli_set_js_passwd(struct cli_param *param, int param_index, char *password)
{
	...
	if ((current_bss->js_passwd[0] != NULL) && (strcmp(current_bss->js_passwd, password) != 0)) {
		/* Remove Jumpstart P2 configuration file if any */
		if (strcmp(current_bss->iface,"ath0") == 0) {
			sprintf(cmdbuf, "rm -Rf %s/js_p2_ath0.conf", WLAN_CFG_PATH);
		} else {
			sprintf(cmdbuf, "rm -Rf %s/js_p2_ath1.conf", WLAN_CFG_PATH);
		}
		cli_exec_cmd(cmdbuf);
	}
...
static int cli_set_factory(struct cli_param *param, int param_index)
{
    ...
    /* Kill wsccmd and hostapd processes */
    sprintf(cmdbuf, "killall wsccmd", WLAN_CFG_PATH);
    cli_exec_cmd(cmdbuf);
    sprintf(cmdbuf, "killall hostapd", WLAN_CFG_PATH);
    cli_exec_cmd(cmdbuf);
    ...
    /* set WSC related defaults */
    sprintf(cmdbuf, "cp %s/default_wsc_cfg.txt %s/wsc_config.txt", 
		WLAN_CFG_DFT_PATH, WLAN_CFG_PATH);
    cli_exec_cmd(cmdbuf);
...
static int cli_save_radio(int unit)
{
    ...
    /* power */
    if (radio->power) {
        cli_save_set_number(fp, &cli_param_list[CLI_PARAM_POWER],radio->power );
    }
...
static int cli_save_wlan(void)
{
    ...
    /* if WDS is enabled */
    cli_save_set_boolean(fp, &cli_param_list[CLI_PARAM_WSC], apcfg->wsc_enabled );

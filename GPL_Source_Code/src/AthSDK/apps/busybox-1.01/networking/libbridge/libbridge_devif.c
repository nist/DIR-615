...
int br_set_guestzone(struct bridge *br, int ifindex, unsigned long int br_ip, int shareport, int route_in_lan)
{
	int gstzone=0;
	
	//printf("br_set_guestzone: ifindex=%d, br_ip=%x, NetUSB=%d, route_in_lan=%d\n", ifindex, br_ip, shareport, route_in_lan);
	...
	if (br_device_ioctl(br, BRCTL_SET_GUEST_ZONE, ifindex, br_ip, gstzone) < 0)
		return errno;

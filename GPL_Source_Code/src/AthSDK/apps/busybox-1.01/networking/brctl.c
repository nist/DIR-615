...
void br_cmd_setguestzone(struct bridge *br, char *ifname, char *arg1)
{
	int err;
	int ifindex;
	unsigned long int br_ip;
	char *shareport=NULL;
	char *route_in_lan=NULL;
	char *ath_name=NULL;
	
	//printf("br_cmd_setguestzone: ifname:usb:route=%s, br_ip=%s\n", ifname, arg1);

	ath_name=strtok(ifname, ":");
	if(ath_name==NULL)
	{
		printf("br_cmd_setguestzone: parser ath_name fail\n");
		return;
	}
	
	shareport = strtok(NULL, ":");
	if(shareport==NULL)
	{
		printf("br_cmd_setguestzone: parser shareport fail\n");
		shareport="1";		
	}
	
	route_in_lan = strtok(NULL, ":");
	if(route_in_lan==NULL)
	{
		printf("br_cmd_setguestzone: parser route_in_lan fail\n");
		route_in_lan="1";
	}
		
	br_ip = inet_addr(arg1);
	if(br_ip == -1)
	{
		printf("br_cmd_setguestzone: parser br_ip fail\n");
		return;
	}
	
	//printf("br_cmd_setguestzone: ath_name=%s, bridge_ip=%x\n", ath_name, br_ip);
	//printf("br_cmd_setguestzone: allow SharePort=%d, allow route_in_lan=%d\n", atoi(shareport), atoi(route_in_lan));
	
	ifindex = if_nametoindex(ath_name);
	if (!ifindex) {
		fprintf(stderr, "setguestzone: interface %s does not exist!\n", ath_name);
		return;
	}

	if ((err = br_set_guestzone(br, ifindex, br_ip, atoi(shareport), atoi(route_in_lan))) == 0)
		return;
	else	
...
static struct command commands[] = {
	...
	{1, 1, "stp", br_cmd_stp},
#ifdef CONFIG_WLAN_GUEST_ZONE
	{1, 2, "setguestzone", br_cmd_setguestzone},
#endif	
};

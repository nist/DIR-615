...
#ifdef CONFIG_FTPPUT
void measure_bandwidth(int time)
{
    int result;
    time++;
    result = (1024/time)*8;
    _system("echo %d > /var/tmp/bandwidth_result.txt",result);
    system("rm -f /var/tmp/test.txt");
}
		...
		const char *server_path, char *local_path)
{
	...
	ftpcmd("QUIT", NULL, control_stream, buf);
	ftime(&end);
	val = end.time - start.time;
	measure_bandwidth(val);
	return(EXIT_SUCCESS);
}
#endif

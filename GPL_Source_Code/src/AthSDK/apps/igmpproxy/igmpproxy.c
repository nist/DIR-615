...
"   -c   Specify a location for the config file (default is '/var/etc/igmpproxy.conf').\n"
...
int main( int ArgCn, const char *ArgVc[] ) {
        ...
        if ( strlen(ArgVc[i]) > 1 && ArgVc[i][0] == '-') {
            ...
            case 'c':
                ...
                if (i + 1 < ArgCn && ArgVc[i+1][0] != '-') {
                    ...
                    log_msg(LOG_ERR, 0, "Missing config file path after -c option.");
                }
                break;
            }
        }
    ...
    IF_DEBUG log_msg(LOG_DEBUG, 0, "Searching for config file at '%s'" , configFilePath);
    ...
    do {
        ...
        if ( !igmpProxyInit() ) {
            log_msg(LOG_ERR, 0, "Unable to initialize IGMPproxy.");
            break;
        }
        ...
        if ( ! debugMode ) {
            ...
            IF_DEBUG log_msg( LOG_DEBUG, 0, "Starting daemon mode.");
            ...
            if ( close( 0 ) < 0 || close( 1 ) < 0 || close( 2 ) < 0 
                ...
                log_msg( LOG_ERR, errno, "failed to detach deamon" );
            }
        }
    ...
    log_msg(LOG_INFO, 0, "Shutdown complete....");
...
int igmpProxyInit() {
    ...
    case 0: break;
    case EADDRINUSE: log_msg( LOG_ERR, EADDRINUSE, "MC-Router API already in use" ); break;
     ...
     */
    {
                ...
                if(Dp->state == IF_STATE_UPSTREAM) {
                    if(upStreamVif[0] == -1) {
                        upStreamVif[0] = Ix;
              			...
              			/*Chun modify: Setting two uptream interface is OK*/
                    	upStreamVif[1] = Ix;                   	
                        //log_msg(LOG_ERR, 0, "Vif #%d was already upstream. Cannot set VIF #%d as upstream as well.",
                        //upStreamVif[0], Ix);
                    }
                }
        ...
        if(vifcount < 2 || ((upStreamVif[0] < 0) && (upStreamVif[1] < 0))) {
            log_msg(LOG_ERR, 0, "There must be at least 2 Vif's where one is upstream.");
        }
    }  
...
void igmpProxyCleanUp(void) {
    ...
    log_msg( LOG_DEBUG, 0, "clean handler called" );
...
void igmpProxyRun() {
	...
	FILE *pid_fp;
	if (!(pid_fp = fopen("/var/run/igmpproxy.pid", "w"))) {
		return ;
	}
    ...
    for (;;) {
        ...
        if (sighandled) {
            if (sighandled & GOT_SIGINT) {
                ...
                log_msg(LOG_NOTICE, 0, "Got a interupt signal. Exiting.");
                break;
            }
            if (sighandled & GOT_SIGUSR1) {
            	sighandled &= ~GOT_SIGUSR1;
                log_msg(LOG_NOTICE, 0, "Got a signal to get group member.");
            	findGroupMember();
            }
        }
        ...
        if( Rt < 0 ) {
            ...
            if( FD_ISSET( MRouterFD, &ReadFDS ) ) {
                ...
                recvlen = recvfrom(MRouterFD, recv_buf, RECV_BUF_SIZE,
                                   0, NULL, &dummy);
                if (recvlen < 0) {
                    if (errno != EINTR) log_msg(LOG_ERR, errno, "recvfrom");
                    continue;
                }

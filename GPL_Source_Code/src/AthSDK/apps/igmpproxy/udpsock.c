...
int openUdpSocket( uint32 PeerInAdr, uint16 PeerPort ) {
    ...
    if( (Sock = socket( AF_INET, SOCK_DGRAM, 0 )) < 0 )
        log_msg( LOG_ERR, errno, "UDP socket open" );
    
    SockAdr.sin_family      = AF_INET;

/* Date: 2009-2-10
*  Name: Jackey Chen
*  Reason: socket open fail with Python solution, replace connect() with bind()
*/
...
#ifdef SUPPORT_BIND
	if(bind(Sock,(struct sockaddr *)&SockAdr,sizeof( SockAdr )) == -1)  
#else    
    if( connect( Sock, (struct sockaddr *)&SockAdr, sizeof( SockAdr ) ) )
#endif
        log_msg( LOG_ERR, errno, "UDP socket connect" );

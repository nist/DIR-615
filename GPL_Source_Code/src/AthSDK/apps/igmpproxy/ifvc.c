     ...
     */
    {
        ...
        for ( IfPt = IfVc; IfPt < IfEp; IfPt++ ) {
            ...
            if (ioctl(Sock, SIOCGIFNETMASK, &IfReq ) < 0)
                log_msg(LOG_ERR, errno, "ioctl SIOCGIFNETMASK for %s", IfReq.ifr_name);
            ...
            log_msg(LOG_DEBUG, 0, "Physical Index value of IF '%s' is %d",
            ...
            */
            if ( ioctl( Sock, SIOCGIFFLAGS, &IfReq ) < 0 )
                log_msg( LOG_ERR, errno, "ioctl SIOCGIFFLAGS" );
            ...
            if(IfDescEp->allowednets == NULL) log_msg(LOG_ERR, 0, "Out of memory !");
            ...
            IF_DEBUG log_msg( LOG_DEBUG, 0, "buildIfVc: Interface %s Addr: %s, Flags: 0x%04x, Network: %s",
                 IfDescEp->Name,
...
int isAdressValidForIf( struct IfDesc* intrface, uint32 ipaddr ) {
    ...
    for(currsubnet = intrface->allowednets; currsubnet != NULL; currsubnet = currsubnet->next) {
        ...
        /*
        IF_DEBUG log_msg(LOG_DEBUG, 0, "Testing %s for subnet %s, mask %s: Result net: %s",
            inetFmt(ipaddr, s1),

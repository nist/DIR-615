...
#if 0 //#ifdef RPPPOE
struct routeconfig*   routeconf;
struct routeconfig *parseRouteToken(void);
#endif
...
int loadConfig(char *configFile) {
...
#if 0 //#ifdef RPPPOE
    struct routeconfig  *tmpPtr_route;
    struct routeconfig  **currPtr_route = &routeconf;
#endif
    ...
    while ( token != NULL ) {
        ...
        if(strcmp("phyint", token)==0) {
            ...
            IF_DEBUG log_msg(LOG_DEBUG, 0, "Config: Got a phyint token.");
            ...
            if(tmpPtr == NULL) {
                ...
                IF_DEBUG log_msg(LOG_DEBUG, 0, "IF name : %s", tmpPtr->name);
                IF_DEBUG log_msg(LOG_DEBUG, 0, "Next ptr : %x", tmpPtr->next);
                IF_DEBUG log_msg(LOG_DEBUG, 0, "Ratelimit : %d", tmpPtr->ratelimit);
                IF_DEBUG log_msg(LOG_DEBUG, 0, "Threshold : %d", tmpPtr->threshold);
                IF_DEBUG log_msg(LOG_DEBUG, 0, "State : %d", tmpPtr->state);
                IF_DEBUG log_msg(LOG_DEBUG, 0, "Allowednet ptr : %x", tmpPtr->allowednets);
#if 0 //#ifdef RPPPOE
                IF_DEBUG log_msg(LOG_DEBUG, 0, "Russia : %d", tmpPtr->russia);                
#endif
            ...
            // Got a route token... Call route parser
            IF_DEBUG log_msg(LOG_DEBUG, 0, "Config: Got a route token.");
            tmpPtr_route = parseRouteToken();
            if(tmpPtr_route == NULL) {
                // Unparsable token... Exit...
                closeConfigFile();
                log_msg(LOG_WARNING, 0, "Unknown token '%s' in configfile", token);
                return 0;
                ...
                IF_DEBUG log_msg(LOG_DEBUG, 0, "Route token ok");
              
                // Insert config, and move temppointer to next location...
                *currPtr_route = tmpPtr_route;
                currPtr_route = &tmpPtr_route->next;
            }
        } 
#endif
        else if(strcmp("quickleave", token)==0) {
            // Got a quickleave token....
            IF_DEBUG log_msg(LOG_DEBUG, 0, "Config: Quick leave mode enabled.");
        ...
        if ( Dp->InAdr.s_addr && ! (Dp->Flags & IFF_LOOPBACK) ) {
            ...
            for( confPtr = vifconf; confPtr; confPtr = confPtr->next) {
                ...
                if(strcmp(Dp->Name, confPtr->name)==0) {
                    ...
                    IF_DEBUG log_msg(LOG_DEBUG, 0, "Found config for %s", Dp->Name);
...
struct vifconfig *parsePhyintToken() {
    ...
    IF_DEBUG log_msg(LOG_DEBUG, 0, "Config: IF: Config for interface %s.", token);
...
//            struct in_addr  networkAddr;
            ...
            token = nextConfigToken();
            IF_DEBUG log_msg(LOG_DEBUG, 0, "Config: IF: Got altnet token %s.",token);
            ...
            *anetPtr = parseSubnetAddress(token);
            ...
            // Upstream
            IF_DEBUG log_msg(LOG_DEBUG, 0, "Config: IF: Got upstream token.");
            tmpPtr->state = IF_STATE_UPSTREAM;
        }
        else if(strcmp("downstream", token)==0) {
            // Downstream
            IF_DEBUG log_msg(LOG_DEBUG, 0, "Config: IF: Got downstream token.");
            tmpPtr->state = IF_STATE_DOWNSTREAM;
        }
#if 0 //#ifdef RPPPOE
        else if(strcmp("physical", token)==0) {
            // Downstream
            IF_DEBUG log_msg(LOG_DEBUG, 0, "Config: IF: Got physical token.");
            tmpPtr->russia = PHYSICAL_IF;
        }
        else if(strcmp("virtual", token)==0) {
            // Downstream
            IF_DEBUG log_msg(LOG_DEBUG, 0, "Config: IF: Got virtual token.");
            tmpPtr->russia = VIRTUAL_IF;
        }
#endif
        else if(strcmp("disabled", token)==0) {
            // Disabled
            IF_DEBUG log_msg(LOG_DEBUG, 0, "Config: IF: Got disabled token.");
            tmpPtr->state = IF_STATE_DISABLED;
        }
        else if(strcmp("ratelimit", token)==0) {
            // Ratelimit
            token = nextConfigToken();
            IF_DEBUG log_msg(LOG_DEBUG, 0, "Config: IF: Got ratelimit token '%s'.", token);
            ...
            // Threshold
            token = nextConfigToken();
            IF_DEBUG log_msg(LOG_DEBUG, 0, "Config: IF: Got threshold token '%s'.", token);
...
#if 0 //#ifdef RPPPOE
/**
*   Internal function to parse route config
*/
struct routeconfig *parseRouteToken() {
    struct routeconfig  *tmpPtr;
    char *route_token;
    short parseError = 0;
    struct in_addr route_ip, route_mask;
    

    // Allocate memory for configuration...
    tmpPtr = (struct routeconfig*)malloc(sizeof(struct routeconfig));
    if(tmpPtr == NULL) {
        log_msg(LOG_ERR, 0, "Out of memory.");
    }
    ...
    // Parse the rest of the config..
    route_token = nextConfigToken();
    while(route_token != NULL) {
        if(strcmp("addr", route_token)==0) {
			route_token = nextConfigToken();                        
            if(route_token == NULL) {
                log_msg(LOG_WARNING, 0, "route address must be inputed");
                parseError = 1;
                break;
            }
            else{
            	IF_DEBUG log_msg(LOG_DEBUG, 0, "Config: IF: Got addr route_token '%s'.", route_token);
            	inet_aton(route_token, &route_ip);
            	tmpPtr->addr = route_ip.s_addr;
            }
        }
        else if(strcmp("mask", route_token)==0) {
            route_token = nextConfigToken();                        
            if(route_token == NULL) {
                log_msg(LOG_WARNING, 0, "route mask must be inputed");
                parseError = 1;
                break;
            }
            else{
            	IF_DEBUG log_msg(LOG_DEBUG, 0, "Config: IF: Got mask route_token '%s'.", route_token);
            	inet_aton(route_token, &route_mask);
            	tmpPtr->mask = route_mask.s_addr;
            }
        }
        else {
            // Unknown route_token. Break...
            break;
        }
        route_token = nextConfigToken();
    }
    ...
    // Clean up after a parseerror...
    if(parseError) {
    	IF_DEBUG log_msg(LOG_DEBUG, 0, "Config: IF: parseError");
        free(tmpPtr);
        tmpPtr = NULL;
    }
...
struct SubnetList *parseSubnetAddress(char *addrstr) {
    ...
    if(tmpStr != NULL) {
        ...
        if(bitcnt <= 0 || bitcnt > 32) {
            log_msg(LOG_WARNING, 0, "The bits part of the address is invalid : %d.",tmpStr);
            return NULL;
        }
    ...
    IF_DEBUG log_msg(LOG_DEBUG, 0, "Config: IF: Altnet: Parsed altnet to %s.",

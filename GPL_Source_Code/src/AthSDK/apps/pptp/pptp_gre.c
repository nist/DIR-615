 ...
 *
 * $Id: pptp_gre.c,v 1.2 2008/02/19 10:35:11 NickChou Exp $
 */
...
int decaps_gre (int fd, callback_t callback, int cl)
{
    ...
    else if ( seq < seq_recv + 1 || WRAPPED(seq_recv, seq) ) 
    {
		...
		if ( log_level >= 1 )
            log("discarding duplicate or old packet %d (expecting %d)",seq, seq_recv + 1);
        stats.rx_underwin++;
    ...
    {	 /* sequence number too high, is it reasonably close? */
		stats.rx_buffered++;
        if ( log_level >= 1 )
            log("%s packet %d (expecting %d, lost or reordered)",  disable_buffer ? "accepting" : "buffering",
                seq, seq_recv+1);
        if ( disable_buffer ) 
        {
            seq_recv = seq;
    ...
    else /* no, packet must be discarded */
    {
		if ( log_level >= 1 )

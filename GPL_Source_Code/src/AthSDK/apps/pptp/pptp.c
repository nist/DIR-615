 ...
 *
 * $Id: pptp.c,v 1.2 2008/02/19 10:35:11 NickChou Exp $
 */
...
int main(int argc, char **argv, char **envp)
{
    ...
    if(launchpppd){
    	...
    	log("Find an open pty/tty pair: ! launchpppd");
    ...
    log("Open connection to call manager");
    do {
        /*
...
shutdown:
    ...
    close(pty_fd);
    close(callmgr_sock);
  sleep(1);     /*NickChou: give ctrl manager a chance to exit */
    exit(0);
}

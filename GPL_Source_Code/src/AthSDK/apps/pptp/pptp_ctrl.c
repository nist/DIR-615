 ...
 *
 * $Id: pptp_ctrl.c,v 1.2 2008/02/19 10:35:11 NickChou Exp $
 */
...
PPTP_CONN * pptp_conn_open(int inet_sock, int isclient, pptp_conn_cb callback)
{
    ...
    if (isclient) {
        ...
        log("Open new pptp_connection: pptp_send_ctrl_packet");
        ...
        char *phonenr)
{
...
#ifdef PPTP_CONN_ID /*NickChou*/    
    char *pptp_connection_id;
	pptp_connection_id = nvram_get("wan_pptp_connection_id");//add by Anderson.Chen
#endif
...
#ifdef PPTP_CONN_ID /*NickChou*/    
    if(pptp_connection_id != NULL && strlen(pptp_connection_id) !=0 )
    {
    	i = atoi(pptp_connection_id);
    	log("wan_pptp_connection_id = %d", i);
    }
#endif
    ...
    log("pptp_call_open: pptp_send_ctrl_packet");
    if (pptp_send_ctrl_packet(conn, &packet, sizeof(packet))) {
        pptp_reset_timer();

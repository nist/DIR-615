...
//#if 0
#if 1
/* uncomment by peter for x509 responder issues
 * (see refine_host_connectionn)
 * */
...
//#if 0
#if 1
    if (!initiator && auth == OAKLEY_RSA_SIG)
    {

    /* 
     * XXX: uncomment by peter, why the source code comment it?
     * In x509, fix @peer_ca is empty as responder?
     * I do not know what will happen when I open it. But It can not find
     * peer_ca when DIR-370 was as responder (It is work fine when DIR-730
     * is initiator) and @trusted_ca will return fail(see below code).
     * 
     * */
    peer_ca = get_peer_ca(peer_id);
	plog("XXX PATCHED X509 responder");

 ...
 * for more details.
 *
 * RCSID $Id: xauth.c,v 1.4 2008/12/26 12:16:09 peter_pan Exp $
 *
...
static int __simple_login (
    const char *conname,
    char *login,
    char *password )
{
#define IPSEC_SECRETS	"/tmp/ipsec.secrets"
#define XAUTH_MAGIC	"#XAUTH:"
    FILE *fd;
    char buf[256], *p = NULL, *user, *pass;
    int rev = -1;

    /*
     *	ex: buf = "#XAUTH:name/u1,p1/u2,p2/ ... /un,pn/"
     * */
    //plog("simple_login(%s, %s)", login, password);
    if ((fd = fopen(IPSEC_SECRETS, "r")) == NULL)
	    return -1;
    while (fgets(buf, sizeof(buf), fd) != NULL) {
	*(strchr(buf, '\n')) = '\0';
	//plog("Retrive:[%s]", buf);
	if (strncmp(buf, XAUTH_MAGIC, strlen(XAUTH_MAGIC)) != 0)
	    continue;
	{
	char *cname;

	p = buf + strlen(XAUTH_MAGIC);
	cname = strsep(&p, "/");
	/*
	 * XXX: in Hub spoke.
	 * @cname: "name", @conname: "name_3" should be matched.
	 * */
	if (strncmp(cname, conname, strlen(cname)) != 0)
	   continue;
	plog("FOUND MATCHED CONN:%s", conname);
	/* FOUND MATCHED CONN */
	/* @p=connection/username,password/username,password/ .. */
	break;
	}
    }
    plog("p=%s", p == NULL?"NULL?":p);
    if (p == NULL)
	return -1;
    do {
    	user = strsep(&p, ",");
	pass = strsep(&p, "/");
	//strsep(&p, "/");
	//plog("fetch:user:%s, pass:%s\n", user, pass);
	
	if (user == NULL || pass == NULL)
		break;
	if (*user == '#' || *pass == '#')
		continue;
	//plog("CAMP: |%s:%s| vs |%s,%s|", user, login, pass, password);
	if (strcmp(user, login) == 0 && strcmp(pass, password) == 0)
		return 0;
    } while (p != NULL && *p != '\0'); /* man 3 strsep */
    return rev;
}
//#define _GNU_SOURCE
//#include <stdio.h>
...
#include <sys/wait.h>
#include <errno.h>
#define IPSEC_LOGIN_HOOK "/etc/sysconfig/ipsec_login"

static int __simple_login_hook (
    const char *conname,
    char *login,
    char *password )
{
	...
	/* 
	 * XXX: For backward compatible: if the hook file not exist
	 * then call old manner
	 * */
	if (access(IPSEC_LOGIN_HOOK, X_OK) == -1)
		return __simple_login(conname, login, password);

	if ((pid = fork()) == -1)
		goto out;
	if (pid == 0) {
		fcloseall();
		setenv("CONNNAME", conname, 1);
		setenv("USERNAME", login, 1);
		setenv("PASSWORD", password, 1);
		execl(IPSEC_LOGIN_HOOK, strrchr(IPSEC_LOGIN_HOOK, '/') + 1, NULL);
		plog("login hook fork: %s", strerror(errno));
		exit(-1);
	}
	...
	if (!WIFEXITED(_ext))
		goto sys_err;
	return WEXITSTATUS(_ext);
	
sys_err:
	plog("xauth login hook %s", strerror(errno));
out:
	return rev;
}
static int simple_login(void *varg)
{
    struct thread_arg	*arg = varg;
    return ! __simple_login_hook(arg->connname.ptr, arg->name.ptr, arg->password.ptr);
}
...
static int xclient_account(const char *connname, char *user, char *pass)
{
	FILE *fd;
	char buf[128], *u, *p;
	int rev = -1;
	
	plog("=========xclient_account(%s)\n", connname);
	sprintf(buf, "/tmp/ipsec.d/%s.xauthcli", connname);
	if ((fd = fopen(buf, "r")) == NULL) {
		if (connname[strlen(connname) - 2] == '_') {
			strcpy(buf, "/tmp/ipsec.d/");
			strncat(buf, connname, strlen(connname) - 2);
			strcat(buf,".xauthcli");
			fd = fopen(buf, "r");
		}
		if (fd == NULL) {
			plog("Cant open %s for xauthclient", buf);
			return -1;
		}
	}
			     ...
			     ,u_int16_t ap_id)
{
    ...
    xclient_account(st->st_connection->name, xauth_username, xauth_password);
    /* START_HASH_PAYLOAD(rbody, ISAKMP_NEXT_ATTR); */
...
/*
 * $Id: xauth.c,v 1.4 2008/12/26 12:16:09 peter_pan Exp $
 *
 * $Log: xauth.c,v $
 * Revision 1.4  2008/12/26 12:16:09  peter_pan
 * bug: xauth hook
 *
 * Revision 1.3  2008/12/22 14:34:24  peter_pan
 * Add XAUTH server hook file for authecticate externally
 *
 * Revision 1.2  2008/10/09 11:37:52  peter_pan
 * support XAUTH server/client
 *
 * Revision 1.1  2008/10/01 03:00:01  ken_chiang
 * *** empty log message ***
 *

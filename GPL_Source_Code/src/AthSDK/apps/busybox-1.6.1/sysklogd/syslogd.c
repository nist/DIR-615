...
#if ENABLE_FEATURE_REMOTE_LOG
#define LOG_IP_FILE "/tmp/log_ip_addr.lst"
#define LOG_IP_MAX 8-1  // the first one is using original busybox's FD
int more_log_ip = 0;
char log_ip_list[8][16] = {{0}};
int log_ip_fd[8];
len_and_sockaddr* remoteAddr[8];

static int get_log_ip_lists(void);
static int get_log_ip_lists(void){
	char buf[1024];
	FILE *fp;
	int i = 0;
	char cmd[256];
	unlink(LOG_IP_FILE);
	memset(log_ip_list,'\0',sizeof(log_ip_list));
	sprintf(cmd,"nvram show |grep log_ipaddr |sed s/.*=// > %s",LOG_IP_FILE);
	system(cmd);
	if((fp=fopen(LOG_IP_FILE,"r"))!=NULL){
		// the first one is using by original busybox's FD
		// so we ignore the first one
		while(1){
			fgets(buf,sizeof(buf),fp);
			if(buf && strlen(buf) > 1 && (-1 != inet_addr(buf))){
				DEBUG_MSG("%s, first log ip is send by original FD, don't record it. %s\n",__FUNCTION__,buf);
				break;
			}
			if(feof(fp)){
				return -1;
		...
		for(i = 0; i < LOG_IP_MAX ; i++){
			...
			memset(log_ip_list[i],'\0',sizeof(log_ip_list[i]));
			fgets(buf,sizeof(buf),fp);
			if(feof(fp)){
				break;
			}
			if(buf && (buf[0] != '\n')){// strlen ("\n ") = 1
				strncpy(log_ip_list[i],buf,sizeof(log_ip_list[i]));
				DEBUG_MSG("%s, entry[%d] : %s",__FUNCTION__,i,buf);
			}
		}
		fclose(fp);
	}else{
	...
	for (;;) {
		...
		if (FD_ISSET(sock_fd, &fds)) {
			...
			if (G.remoteAddr) {
				...
				{
					int x = 0;
					DEBUG_MSG("%s, there are multiple remote log ip \n",__FUNCTION__);
					for(x = 0 ; x < LOG_IP_MAX ; x++ ){
						//if(log_ip_list[x] && (strlen(log_ip_list[x]) > 1)){ // strlen("\n") = 1
						if(-1 != inet_addr(log_ip_list[x])){
						//if(0 != inet_aton(log_ip_list[x],&inp)){
							DEBUG_MSG("%s, valid remote log ip [%s]\n",__FUNCTION__,log_ip_list[x]);
							if(-1 == log_ip_fd[x]){
								DEBUG_MSG("%s, invalid log_ip_fd[%d], socket again\n",__FUNCTION__,x);
								log_ip_fd[x] = socket(remoteAddr[x]->sa.sa_family, SOCK_DGRAM, 0);
							}
							if(-1 != log_ip_fd[x]){
								DEBUG_MSG("%s, send log msgs to log_ip_fd[%d]\n",__FUNCTION__,x);
								if(-1 == sendto(log_ip_fd[x], G.recvbuf, i, MSG_DONTWAIT,
										&remoteAddr[x]->sa, remoteAddr[x]->len)){
									DEBUG_MSG("%s, send log msgs to log_ip_fd[%d] failed (%d)!!!\n",__FUNCTION__,x,errno);
								}
							}else{
								DEBUG_MSG("%s, invalid log_ip_fd[%d], can't re-socket again !!!\n",__FUNCTION__,x);
							}
						}else{
							DEBUG_MSG("%s, invalid remote log ip [%s], ignore\n",__FUNCTION__,log_ip_list[x]);
						}
					}
				}else{
					DEBUG_MSG("%s, there are NO multiple remote log ip \n",__FUNCTION__);
				}
				/* -------------------------------- */
			}
#endif
...
int syslogd_main(int argc, char **argv)
{
	...
	if (option_mask32 & OPT_remotelog) { // -R
		...
		/* jimmy added for remote log */
		memset(log_ip_fd,-1,sizeof(log_ip_fd));
		if(-1 != get_log_ip_lists()){
			for(i = 0 ; i < LOG_IP_MAX ; i++){
				if(log_ip_list[i] && (strlen(log_ip_list[i]) > 0) && (log_ip_list[i][0] != '\n')){
					DEBUG_MSG("%s, [%d] remote log ip : [%s], len = %d\n"
								,__FUNCTION__,i,log_ip_list[i],strlen(log_ip_list[i]));
					more_log_ip = 1;
					remoteAddr[i] = xhost2sockaddr(log_ip_list[i],514);
				}
			}
		}
		/* -------------------------------- */

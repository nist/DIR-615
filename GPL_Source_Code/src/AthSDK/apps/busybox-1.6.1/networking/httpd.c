 ...
 */
#if ENABLE_FEATURE_HTTPD_BASIC_AUTH
static int checkPerm2(const char *url, const char *uidpwd)
{
	int ret = 1;
	char *p, tmp_uidpwd[128], uid[64], pwd[64];
#ifdef GRAPHICAL_AUTH
	cprintf("ok pass anyways, %s,%d\n", config->rmt_ip_str, config->port);
...
static int is_user_in_http_and_ssl(const char *user)
{
	FILE *fp = fopen("/tmp/https.conf", "r");
	char str[80];
	char uname[80];
	int count = 0;
	...
	sprintf(uname, ":%s:", user);
	while(fscanf(fp, "%s", str) != EOF)
		if(strstr(str, uname) != NULL)
			count++;
...
static int
remaping_url(const char *purl)
{
	int rev = -1;
	char *buf = config->buf;

	struct {
		const char *purl;	// original url
		const char *remap;	// remaped url
	} *p, rmp[] = {
		{ "/version.txt", "/cgi/ssi/tmp/version.txt" },
		{ "/chklst.txt", "/cgi/ssi/tmp/chklst.txt" },
		{ "/wlan.txt", "/cgi/ssi/tmp/wlan.txt" },
		{ "/reject.html", "/cgi/ssi/tmp/deny.html" },
		{ NULL, NULL }
	};

	for (p = rmp; p->purl != NULL; p++) {
		if (strncmp(purl, p->purl, strlen(p->purl)) == 0) {
			if (str_replace(buf, p->remap, p->purl, sizeof(config->buf)) != 0)
				return rev;
			syslog(LOG_INFO, "remaping [%s]\n", buf);
		}
	}
...
static int check_access_perm(const char *user, char *url)
{
	cprintf("url: %s\n", url);
	if (allowed_page(url, __func__) == 1)
		return 1;
...
#ifdef GRAPHICAL_AUTH
	if (auth_session_exist(config->rmt_ip_str) != 0 ||
	    auth_session_timeout(config->rmt_ip_str) != 0)
		strcpy(url, "/cgi/ssi/login_pic.asp");

	return 1;
#else
	if(is_user_in_http_and_ssl(user))	// both sslvpn account and admin account
		return 1;

	if(is_user_in_http(user) == 1 && strstr(url, "ssl/"))
		return 0;

	if(is_user_in_http(user) == 0 && strstr(url, "remote_decision.asp"))
		return 1;

	if(is_user_in_http(user) == 0 && strstr(url, "ssl/") == NULL)
		return 0;
...
static int sendHeaders(HttpResponseNum responseNum)
{
	...
	return config->full_write(config, buf, len);
}

static ssize_t hconf_read(HttpdConfig *it, void *buf, size_t count)
{
#if ENABLE_FEATURE_HTTPD_SSL
	return (it->do_ssl && it->ssl != NULL) ?
		SSL_read(it->ssl, buf, count): read(it->accepted_socket, buf, count);
#else
	return read(it->accepted_socket, buf, count);
#endif
}
#if ENABLE_FEATURE_HTTPD_SSL
static inline ssize_t hconf_ssl_safe_read(HttpdConfig *it, void *buf, size_t count)
{
	ssize_t n;

	do {
		n = SSL_read(it->ssl, buf, count);
	} while (n < 0 && errno == EINTR);

	return n;
}
#endif /* ENABLE_FEATURE_HTTPD_SSL */
static inline ssize_t hconf_safe_read(HttpdConfig *it, void *buf, size_t count)
{
#if ENABLE_FEATURE_HTTPD_SSL
	return (it->do_ssl && it->ssl != NULL) ?
		hconf_ssl_safe_read(it, buf, count):
		safe_read(it->accepted_socket, buf, count);
#else
	return safe_read(it->accepted_socket, buf, count);
#endif
}
static inline ssize_t hconf_full_read(HttpdConfig *it, void *buf, size_t len)
{
	ssize_t cc;
	ssize_t total;
...
static ssize_t hconf_write(HttpdConfig *it, const void *buf, size_t len)
{
#if ENABLE_FEATURE_HTTPD_SSL
		return (it->do_ssl && config->ssl != NULL) ?
			SSL_write(it->ssl, buf, len):
			write(it->accepted_socket, buf, len);
#else
		return write(it->accepted_socket, buf, len);
#endif
}

#if ENABLE_FEATURE_HTTPD_SSL
static inline ssize_t hconf_ssl_safe_write(HttpdConfig *it, const void *buf, size_t count)
{
	ssize_t n;

	do {
		n = SSL_write(it->ssl, buf, count);
	} while (n < 0 && errno == EINTR);

	return n;
}
#endif /* ENABLE_FEATURE_HTTPD_SSL */
static ssize_t hconf_safe_write(HttpdConfig *it, const void *buf, size_t len)
{
#if ENABLE_FEATURE_HTTPD_SSL
		return (it->do_ssl && config->ssl != NULL) ?
			hconf_ssl_safe_write(it, buf, len):
			safe_write(it->accepted_socket, buf, len);
#else
		return safe_write(it->accepted_socket, buf, len);
#endif
}
static ssize_t hconf_full_write(HttpdConfig *it, const void *buf, size_t len)
{
	ssize_t cc;
	ssize_t total;
...
static int getLine(void)
{
	...
	while (config->read(config, buf + count, 1) == 1) {
 ...
 *
 ****************************************************************************/
static int sendCgi(char *url,
		const char *request, int bodyLen, const char *cookie,
		const char *content_type)
{
#if 0
	FILE *fp = fopen("/tmp/httpdlog", "a+");
	fprintf(fp, "sendCgi: %s %s\n", config->remoteuser, url);
	fclose(fp);
#endif
	if(check_access_perm(config->remoteuser, url) == 0) {
		sendHeaders(HTTP_NOT_FOUND);
		_exit(242);
	}

	cprintf("sendCgi url = %s\n", url);
	system("echo \"sendCgi flag\" >> /tmp/httpdlog");
	...
	system("echo \"sendCgi flag2\" >> /tmp/httpdlog");
 ...
 */
	...
	if (!pid) {
		/* child process */
		system("echo \"sendCgi flag3\" >> /tmp/httpdlog");
		char *script;
		char *purl;
		...
		*/
		...
		setenv1("PATH_INFO", script);   /* set /PATH_INFO or "" */
		{
			...
			getcwd(trans_info, sizeof(trans_info));
			strcat(trans_info, script);
			setenv1("PATH_TRANSLATED", trans_info);
		}
 ...
 error_execing_cgi:
	...
	system("echo \"sendCgi flag4\" >> /tmp/httpdlog");
	buf_count = 0;
	post_read_size = 0;
	...
	while (1) {
		...
		/*
		syslog(LOG_INFO, "nfound:%d", nfound);
		if (FD_ISSET(outFd, &writeSet))
			syslog(LOG_INFO, "outFd Write set, post_read_size = %d", post_read_size);
		if (FD_ISSET(config->accepted_socket, &readSet))
			syslog(LOG_INFO, "accept_socket read set");
		if (FD_ISSET(inFd, &readSet))
			syslog(LOG_INFO, "inFd read set");
		*/	
		if (nfound <= 0) {
...
#if ENABLE_FEATURE_HTTPD_SSL
		/* For SSL socket, SSL Socket layer might modified I/O attr of FD,
		 * I think that it will make the @select invalid.
		 * */
		if (FD_ISSET(outFd, &writeSet)) {
			if (post_read_size <= 0)
				goto read_sock;
		...
		// (imagine that CGI does not read its stdin...)
			...
			//syslog(LOG_INFO, "To CGI:%d, %d,%d, %d", count, post_read_size, post_read_idx, bodyLen);
...
read_sock:
			...
			//syslog(LOG_INFO, "bodyLen:%d want read", bodyLen);
			count = config->safe_read(config, wbuf, count);
			//syslog(LOG_INFO, "bodyLen:%d read", count);
			if (count > 0) {
				post_read_size = count;
				post_read_idx = 0;
				bodyLen -= count;
		...
		if (FD_ISSET(inFd, &readSet)) {
			...
			if (buf_count >= 0) {
				...
				if (count <= 0) {
					...
					if (buf_count) {
						/* XXX: How about inetd ?, should it call config->write ? */
						config->full_write(config, HTTP_200, sizeof(HTTP_200)-1);
						config->full_write(config, rbuf, buf_count);
					}
					break;  /* closed */
				}
				...
				if (buf_count >= 4) {
					...
					if (memcmp(rbuf, HTTP_200, 4) != 0) {
						...
						if (config->full_write(config, HTTP_200, sizeof(HTTP_200)-1) != sizeof(HTTP_200)-1)
							break;
					}
			...
			if (config->full_write(config, rbuf, count) != count)
				break;
			if (DEBUG)
 ...
 *
 ****************************************************************************/
static int sendFile(char *url)
{
#if 0
	FILE *fp = fopen("/tmp/httpdlog", "a+");
	fprintf(fp, "sendFile: %s %s\n", config->remoteuser, url);
	fclose(fp);
#endif
	if(check_access_perm(config->remoteuser, url) == 0) {
		sendHeaders(HTTP_NOT_FOUND);
	...
	cprintf("sendFile url = %s\n", url);
#if 0	//FEATURE_HTTPD_GRAPH_AUTH
	cprintf("url = %s\n", url);
	cprintf("config->rmt_ip_str: %s\n", config->rmt_ip_str);
	if (access(config->rmt_ip_str, F_OK) != 0) {
		url = "redirect2login.html";	//this pages only "relocation:" to login page as unauthorized user.
		cprintf("redirect url = %s\n", url);
	}
#endif
	char * suffix;
	int  f;
	...
	if (f >= 0) {
		...
		while ((count = full_read(f, buf, MAX_MEMORY_BUFF)) > 0) {
			 ...
			 *	break;
			 *	FIXME: I ignored inetd issues!
			 */
			if (config->full_write(config, buf, count) != count)
				break;
		}
		close(f);
	} else {
...
static void HttpdConfig_init(HttpdConfig *it)
{
	it->write = hconf_write;
	it->safe_write = hconf_safe_write;
	it->full_write = hconf_full_write;
	it->read = hconf_read;
	it->safe_read = hconf_safe_read;
	it->full_read = hconf_full_read;
}
...
static void handleIncoming(void)
{
	...
	HttpdConfig_init(config);

#if ENABLE_FEATURE_HTTPD_SSL 

	//config->cipher = (const char *)("ALL:!RC4");
	//config->cipher = (const char *)("ALL:!ADH:!LOW:!EXP:!MD5:@STRENGTH");
	//config->cipher = (const char *)0;
	...
	config->ssl_ctx = SSL_CTX_new( SSLv23_server_method() );
	if ( config->certfile[0] != '\0' )
	    if ( SSL_CTX_use_certificate_file( config->ssl_ctx, config->certfile, SSL_FILETYPE_PEM ) == 0 ||
		 SSL_CTX_use_PrivateKey_file( config->ssl_ctx, config->certfile, SSL_FILETYPE_PEM ) == 0 ||
		 SSL_CTX_check_private_key( config->ssl_ctx ) == 0 )
		{
		syslog(LOG_INFO, "load pem error");
		ERR_print_errors_fp( stderr );
		exit( 1 );
		}
	if ( config->cipher != (char*) 0 )
	    {
	    if ( SSL_CTX_set_cipher_list( config->ssl_ctx, config->cipher ) == 0 )
		{
		syslog(LOG_INFO, "cipher error");
		ERR_print_errors_fp( stderr );
		return;
		}
	    }
	/* Accepted */
	config->ssl = SSL_new( config->ssl_ctx );
	if (SSL_set_fd( config->ssl, config->accepted_socket) != 1) {
		syslog(LOG_INFO, "SSL_set_fd err");
		return;
	}
	if ((ssl_ret = SSL_accept( config->ssl )) != 1 )
	    {
		syslog(LOG_INFO, "SSL_accept no list:%d err Return", ssl_ret);
		switch ((ssl_ret = SSL_get_error(config->ssl, ssl_ret)))
		{
		case SSL_ERROR_SSL:
			syslog(LOG_INFO, "SSL protocol error");
			break;
		case SSL_ERROR_ZERO_RETURN:
			syslog(LOG_INFO, "SSL ZERO RETURN ERROR?");
			break;
		case SSL_ERROR_SYSCALL:
			syslog(LOG_INFO, "CALL back return");
			break;
		default:
			syslog(LOG_INFO, "SSL default ERROR:%d", ssl_ret);
		}
/*
		{
			FILE *sfd;
			char bbuf[512];
			ERR_error_string(ERR_get_error(), bbuf);
			syslog(LOG_INFO, "SSL accept:%s", bbuf);
			sfd = fopen("/tmp/ssl.log", "a");
	    	ERR_print_errors_fp(sfd);
			fclose(sfd);
		}
*/
	    return;
	...
	//openlog("bzhttpd", LOG_PID, LOG_USER);
#endif /* ENABLE_FEATURE_HTTPD_SSL */
	do {
		int count;
...
#define SKIP_SPACE(p) while (isspace(*(p)) && *(p) != '\0'){(p)++;}
			const char *r, *map = config->map_url;

			if (map == NULL || (r = strrchr(map, '/')) == NULL)
				goto no_map;
			
			if ((purl = strpbrk(buf, " \t")) == NULL)
				goto no_map;
			
			SKIP_SPACE(purl);
/*
			if (strncmp(purl, "/chklst.txt", 11) == 0) {
				if (str_replace(buf, "/cgi/ssi/tmp/version.txt", "/chklst.txt", sizeof(config->buf)) != 0)
					goto no_map;
				syslog(LOG_INFO, "remaping [%s]\n", buf);
			}

			if (strncmp(purl, r, strlen(r)) == 0) {
				// FIXME: Do we need support "POST" method ?
				if (str_replace(buf, map, r, sizeof(config->buf)) != 0)
					goto no_map;
				syslog(LOG_INFO, "remaping [%s]\n", buf);
			}
...
*/
			...
			/* if url remap to another url, return 0, otherwise return -1 */
			if (remaping_url(purl) != 0)
				goto no_map;
		}
no_map:
#endif /* ENABLE_FEATURE_HTTPD_MAPPING_URL */
		purl = strpbrk(buf, " \t"); /* ex @buf = "GET /URL HTTP/1.1\n\n" */
		if (purl == NULL) {
 BAD_REQUEST:
			sendHeaders(HTTP_BAD_REQUEST);
			break;
		}
		*purl = '\0';	/* ex @buf = "GET\0 /URL HTTP/1.1\n\n" */
#if ENABLE_FEATURE_HTTPD_CGI
		...
		*purl = ' ';	/* ex @purl = " /URL HTTP/1.1\n\n" */
		...
		url = alloca(strlen(buf) + sizeof("/"INDEX_PAGE));
		if (url == NULL) {
		...
		if (blank >= 0) {
			...
			while (1) {
				...
				if (STRNCASECMP(buf, "Authorization:") == 0) {
					...
					credentials = checkPerm2(url, test);
				}
#endif          /* FEATURE_HTTPD_BASIC_AUTH */
 ...
 FORBIDDEN:		/* protect listing /cgi-bin */
		...
		if (credentials <= 0 && checkPerm2(url, ":") == 0) {
...
#if 0
		FILE *fp = fopen("/tmp/httpdlog", "a+");
		fprintf(fp, "handleIncoming: %s %s %s\n", config->remoteuser, test, config->query);
		fclose(fp);
#endif
		if (strncmp(test, "cgi-bin", 7) == 0) {
			...
			break;
		} else if (strncmp(test, "cgi", 3) == 0) {
			if (test[3] == '/' && test[4] == 0)
				goto FORBIDDEN;     /* protect listing cgi-bin/ */
			sendCgi(url, prequest, length, cookie, content_type);
			break;
		} else if (strncmp(test, "tmp", 3) == 0) {
			if (test[3] == '/' && test[4] == 0)
				goto FORBIDDEN;     /* protect listing cgi-bin/ */
			sendCgi(url, prequest, length, cookie, content_type);
			break;
		...
		if (stat(test, &sb) == 0) {
			...
			if (access("/cgi-bin/index.cgi"+1, X_OK) == 0) {
				char turl[128] = "/cgi-bin/index.cgi";

				purl[0] = '\0';
				config->query = url;
				sendCgi(turl, prequest, length, cookie, content_type);
				break;
			}
		}
#endif  /* FEATURE_HTTPD_CGI */
...
# endif
	...
	} while (retval > 0 && config->read(config, buf, sizeof(config->buf) > 0));
...
enum {
	...
	USE_FEATURE_HTTPD_SSL(           k_opt_cert      ,)
	USE_FEATURE_HTTPD_MAPPING_URL(   M_opt_map_url   ,)
	p_opt_port      ,
	p_opt_inetd     ,
	p_opt_foreground,
	p_opt_ssl,
	...
	OPT_CERT		= USE_FEATURE_HTTPD_SSL(           (1 << k_opt_cert      )) + 0,
	OPT_MAP_URL		= USE_FEATURE_HTTPD_MAPPING_URL(   (1 << M_opt_map_url   )) + 0,
...
int httpd_main(int argc, char **argv)
{
...
#if ENABLE_FEATURE_HTTPD_MAPPING_URL
	#if	ENABLE_FEATURE_HTTPD_MAPPING_STRING
	config->map_url = CONFIG_FEATURE_HTTPD_MAPPING_STRING;
	#endif
#endif
	opt = getopt32(argc, argv, "c:d:h:"
			...
			USE_FEATURE_HTTPD_SSL("k:")
			USE_FEATURE_HTTPD_MAPPING_URL("M:")
			"p:if"
			USE_FEATURE_HTTPD_SSL("S"),
			&(config->configFile), &url_for_decode, &home_httpd
			USE_FEATURE_HTTPD_ENCODE_URL_STR(, &url_for_encode)
			USE_FEATURE_HTTPD_BASIC_AUTH(, &(config->realm))
			USE_FEATURE_HTTPD_AUTH_MD5(, &pass)
			USE_FEATURE_HTTPD_SETUID(, &s_ugid)
			USE_FEATURE_HTTPD_SSL(, &config->certfile)
			USE_FEATURE_HTTPD_MAPPING_URL(, &config->map_url)
			, &s_port
		);
	...
	if (opt & OPT_SSL) {
		config->do_ssl = 1;
		if (access(config->certfile, F_OK) != 0) {
			printf("Certificate %s Not Found!", config->certfile);
			return 0;
		}
	}

	openlog("httpd", LOG_PID, LOG_USER);
	printf("openlog\n");
	syslog(LOG_INFO, "https support");
#endif

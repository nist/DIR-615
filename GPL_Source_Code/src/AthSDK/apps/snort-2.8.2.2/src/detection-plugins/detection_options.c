/* $Id: detection_options.c,v 1.1 2008/10/01 06:17:13 ken_chiang Exp $ */
/*
...
static void detection_option_node_update_otn_stats(detection_option_tree_node_t *node, node_profile_stats_t *stats)
{
    int i;
    ...
    if (node->num_children)
    {
        for (i=0;i<node->num_children; i++)
        {
            detection_option_node_update_otn_stats(node->children[i], &local_stats);
        }
    }
}

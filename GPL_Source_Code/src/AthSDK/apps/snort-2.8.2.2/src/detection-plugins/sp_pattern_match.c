...
/* $Id: sp_pattern_match.c,v 1.1 2008/10/01 06:17:13 ken_chiang Exp $ */
...
void FinalizeContentUniqueness(OptTreeNode *otn)
{
    ...
    while (opt_fp)
    {
            ...
            (opt_fp->OptTestFunc == CheckUriPatternMatch))
        {
            ...
            if (add_detection_option(option_type, (void *)pmd, (void **)&pmd_dup) == DETECTION_OPTION_EQUAL)
            {
#if 0
...
int PatternMatchAdjustRelativeOffsets(PatternMatchData *pmd, const u_int8_t *orig_doe_ptr, const u_int8_t *start_doe_ptr, const u_int8_t *dp)
{
    ...
    if (orig_doe_ptr)
    {
        if (((pmd->distance != 0) && (start_doe_ptr - orig_doe_ptr > pmd->distance)) ||
            ((pmd->offset != 0) && (start_doe_ptr - orig_doe_ptr > pmd->offset)))
        {
        ...
        if (((pmd->within != 0) && (start_doe_ptr - orig_doe_ptr + pmd->pattern_size > pmd->within)) ||
            ((pmd->depth != 0) && (start_doe_ptr - orig_doe_ptr + pmd->pattern_size > pmd->depth)))
        {
    ...
    else
    {
        ...
        if (((pmd->within != 0) && (start_doe_ptr - dp + pmd->pattern_size > pmd->within)) ||
            ((pmd->depth != 0) && (start_doe_ptr - dp + pmd->pattern_size > pmd->depth)))
        {
            /* This was within to a previously found pattern.
             * No space left to search, we're done */
            retval = 0;

					  ...
					  u8 *eap, size_t len)
{
	...
	//printf("NickChou: auth_server->index = %d, addr = %s\n", hapd->conf->radius->auth_server->index, hostapd_ip_txt(&hapd->conf->radius->auth_server->addr, abuf, sizeof(abuf)));
	...
	if( (hapd->conf->mac_auth == hapd->conf->radius->auth_server->index) || (hapd->conf->mac_auth == 3 ) )	/**NickChou add for MAC-Auth. 070622**/
	{
	...
	if (!radius_msg_add_attr(msg, RADIUS_ATTR_CALLING_STATION_ID,
		...
		printf("MAC Auth. is enable, add Calling-Station-Id\n");
	}	
	else
		printf("MAC Auth. is disable, don't add Calling-Station-Id\n");

...
#define RADIUS_CLIENT_MAX_ENTRIES 30 /* maximum number of entries in retransmit
				      ...
				      * limit is exceeded) */
#define RADIUS_CLIENT_NUM_FAILOVER  2 /*4  try to change RADIUS server after this
...
static void radius_client_receive(int sock, void *eloop_ctx, void *sock_ctx);/*Chun add: for 2-RADIUS-SERVERS-NETWORK */
...
static void radius_client_timer(void *eloop_ctx, void *timeout_ctx)
{
	...
	char abuf[50];
	int ok = 0;		/*Chun add: for 2-RADIUS-SERVERS-NETWORK */
	int shutdown_ok = 0;	/*Chun add: for 2-RADIUS-SERVERS-NETWORK */
	...
	if (auth_failover && conf->num_auth_servers > 1) {
		...
		/*Chun add: for 2-RADIUS-SERVERS-NETWORK */
		/* TODO: 
		 * Close the previous socket, or we may run out of sockets 
		 * if there is no radius server is reachable 
		 */
		//if(shutdown(radius->auth_serv_sock,2))
		//	printf("SOCKET: shutdown socket fail\n");
		
		radius->auth_serv_sock = socket(PF_INET, SOCK_DGRAM, 0);
		
		if (radius->auth_serv_sock < 0)
			perror("[radius_client_timer]");
		else
		{	printf("open radius socket success\n");
			ok++;
		}

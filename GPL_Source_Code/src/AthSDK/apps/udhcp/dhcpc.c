...
*/		
	...
	while (1) {
		int option_index = 0;
		c = getopt_long(argc, argv, "c:CV:fbH:h:F:i:w:np:qr:s:v:uNRaA", arg_options, &option_index);
		if (c == -1) break;
...
*/		
#ifdef AUTO_MODE_SELECT	
					//LOG(LOG_INFO, "Auto mode select = %x", client_config.auto_mode_select);
					//LOG(LOG_INFO, "Wan protocol = %s ", client_config.wan_proto);
					if( (strcmp(client_config.wan_proto, "dhcpc") == 0) && (client_config.auto_mode_select) ){						
						//LOG(LOG_INFO, "Lease of %x obtained", packet.yiaddr);
						//LOG(LOG_INFO, "client_config.ap_mode=%d",client_config.ap_mode);
						previous_ip = (packet.yiaddr & inet_addr("255.0.0.0"));							
						previous_ip = previous_ip>>24;						
						if( (previous_ip== 0x0a) || (previous_ip== 0xac) || (previous_ip== 0xc0)){// 10,172,192
							LOG(LOG_INFO, "dhcpc get private ip");
							if(!client_config.ap_mode){
								LOG(LOG_INFO, "To change router to AP mode");
								_system("nvram set wlan0_mode=ap");
								_system("nvram set wan_proto=static");																								
								_system("nvram set wan_static_ipaddr=192.168.0.50");
								_system("nvram set wan_static_netmask=255.255.255.0");
							...
							LOG(LOG_INFO, "dhcpc get public ip");
							if(client_config.ap_mode){

...
void manual_only(int sig)
{
	if(checkServiceAlivebyName("monitor"))
		system("killall monitor");
	usleep(500);
	if(checkServiceAlivebyName("redial"))
		system("killall redial");
	usleep(500);
	if(checkServiceAlivebyName("ppptimer"))
		system("killall -9 ppptimer");
        return ;
}
void always_only(int sig)
{
	if(checkServiceAlivebyName("monitor"))
	        system("killall monitor");
	usleep(500);
	if(checkServiceAlivebyName("ppptimer"))
		system("killall -9 ppptimer");
	usleep(500);
	if(checkServiceAlivebyName("redial") == 0)
		system("/var/sbin/redial 10 &");
        return ;
}
void demand_only(int sig)
{
	if(checkServiceAlivebyName("redial"))
	        system("killall redial");
	usleep(500);
        if(checkServiceAlivebyName("monitor") == 0)
                system("/var/sbin/monitor 5 &");
	usleep(500);
	if(checkServiceAlivebyName("ppptimer") == 0)
                system("/var/sbin/ppptimer &");
	else
		system("killall -SIGSYS ppptimer");

        return ;
}
void always_demand(int sig)
{
	if(checkServiceAlivebyName("redial") == 0)
                system("/var/sbin/redial 10 &");
	usleep(500);
...
event_mainloop(void)
{
    ...
    {
#ifdef MPPPOE
	signal(SIGUSR1, manual_only);
        signal(SIGUSR2, always_only);
	signal(SIGHUP, demand_only);
	signal(SIGTSTP, always_demand);
#endif

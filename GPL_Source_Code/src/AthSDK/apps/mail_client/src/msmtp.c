...
/* Date: 2009-06-17
 * Name: Cosmo
 * Reason: update the log-emailing status
 * Usage: 
 		updateEmailStatus(EMAIL_LOG_SUCCESS)	-->   update status to "success(1)"
 		updateEmailStatus(EMAIL_LOG_FAIL)		-->   update status to "fail(0)"
 		updateEmailStatus(EMAIL_LOG_STATUS_INIT)-->   checking if in "init" state.
 														- If yes, update status to "fail(0)"
 														- If not, keep the previous state					
 */
void updateEmailStatus(int email_status){	
	
	int no_need_to_update = 0;
	char execEmailStatusBuf[40] = {};
	FILE *fp = fopen(STATUS_OF_EMAIL_LOG,"r");	
	char line[2] = {};	
	...
	switch(email_status){
		case EMAIL_LOG_SUCCESS:			
		case EMAIL_LOG_FAIL:						
			sprintf(execEmailStatusBuf,"echo %d > %s",email_status,STATUS_OF_EMAIL_LOG);
			system(execEmailStatusBuf);
			break;
		case EMAIL_LOG_STATUS_INIT:
			/* check if the previous result has been written or not */
			/* if not, need to update the result to "fail" */
			if(!no_need_to_update){					
				sprintf(execEmailStatusBuf,"echo %d > %s",EMAIL_LOG_FAIL,STATUS_OF_EMAIL_LOG);
				system(execEmailStatusBuf);
			}	
			break;
		default:				
			sprintf(execEmailStatusBuf,"echo %d > %s",EMAIL_LOG_STATUS_INIT,STATUS_OF_EMAIL_LOG);
			system(execEmailStatusBuf);
			break;			
	}
}
...
send_without_auth:
	...
	* Date: 2009-06-17
	* Name: Cosmo
	* Reason: update the status while sending mail success
	*/		
	updateEmailStatus(EMAIL_LOG_SUCCESS);
...
//		_system("rm -rf %s",LOG_FILE_FULL);
		char execBuf[40] = {};
		sprintf(execBuf, "rm -rf %s",LOG_FILE_FULL); 	
		system(execBuf);
...
exit:
	/* Date: 2009-06-17
 	* Name: Cosmo
 	* Reason: Update the log-emailing status from "init" to "fail" if the previous status is "init". 		
 	*/	
	updateEmailStatus(EMAIL_LOG_STATUS_INIT);
    /* clean up */

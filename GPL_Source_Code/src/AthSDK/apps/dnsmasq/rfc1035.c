...
#ifdef MPPPOE
int catch_dns(HEADER *header, char *limit, unsigned int qlen,char *name )
{
	unsigned char *p, *ansp;
  	char ipaddr[32]={},temp_lan_ip[32]={};
  	struct in_addr lan_ip;
  	unsigned int nameoffset;
  	int ans, anscount = 0, return_flag = 1;
  	unsigned long ttl = 0 ;
  	FILE *fp;
	char buf[256] = {};
	char *domain_name;
	...
	fp = fopen("/var/tmp/domain_records","r");
	if(fp)
	{
		while(fgets(buf,256,fp))
		{
			...
			{
				return_flag = 0;
				domain_name = strtok(buf,"/");
				strcpy(temp_lan_ip,strtok(NULL,"/"));
				temp_lan_ip[strlen(temp_lan_ip) - 1] = '\0';	
				break;			
			}
	...
	if(return_flag)//this query does not match any domain name query
		return 0;
	//device can not resolve domain name, just forward this packet to internet
	if((strcmp(temp_lan_ip,"UNKNOW HOST") == 0) || (inet_addr(temp_lan_ip) != -1))
		return 0;	
	strcpy( ipaddr , temp_lan_ip);
        ans = 1;
        /* copy question as first part of answer (use compression) */
        PUTSHORT(nameoffset | 0xc000, ansp);
        PUTSHORT(T_A, ansp);

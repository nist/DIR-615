...
#ifdef MPPPOE
int search_mapping_domain_name(char *domain_name)
{
	FILE *fp;
	char content[256] = {};
	char *interface_name;
	fp = fopen("/var/tmp/dns_mpppoe","r");
	if(fp)
	{
		while(fgets(content,256,fp))
		{
			if(strstr(content,domain_name))
			{
				interface_name = strtok(content,"/");
				interface_name = interface_name + 3;
				if_unit = atoi(interface_name);
				fclose(fp);
				return 1;
			}
			else
		...
		fseek(fp,0,SEEK_SET);
		interface_name = fgets(content,256,fp);
		if_unit = atoi(interface_name);
		fclose(fp);
		return 0;
	}
...
char *return_wan_proto(void)
{
	FILE *fp;
        char buff[128] = {};
        char *lan,*wan,*wlan,*hostname,*wan_proto;
        fp = fopen("/var/tmp/interface_info","r");
        if(fp)
        {
                fgets(buff,128,fp);
                lan = strtok(buff,"/");
			     ...
			     struct server *servers, struct server *last_server)
{
...
#ifdef MPPPOE  
  char resolve_file[] = "/var/etc/resolve_XY";
  search_mapping_domain_name(dnamebuff);
#endif
  ...
  if (forward)
	...
	if(strcmp(return_wan_proto(),"pppoe") == 0)
	{
		sprintf(resolve_file,"/var/etc/resolve_%02d",if_unit);
		reload_servers(resolve_file,temp,forward->sentto);	
		firstsentto = forward->sentto;
		//syslog(LOG_INFO,"ifunit = %d",if_unit);
          	//syslog(LOG_INFO,"resolve_file = %s",resolve_file);
	}	
#else

...
my @adv_options = (
	'NO_INLINE', 'NO_INET6_OPTION'
);

my %adv_options_desc = (
	'NO_INET6_OPTION'	=> 'Support old LibC, include inet6 option processing',
...
sub write_config {
	...
	print F "GENERATED_MODULE_LIST = yes\n";
	...
	print F "STATIC_MODULE_SOURCES =";

	foreach (@mrd_modules) {
		if ($static_modules{$_}) {
			print F ' $(', (uc $_), '_SOURCES)';
		}
	}
	...
	if ($adv_options_values{'NO_INET6_OPTION'}) {
		print F "NO_INET6_OPTION = yes\n";
	}
...
sub write_modules_cpp {
	if (!open F, '> src/modules.generated.cpp') {
		print 'Failed to open src/modules.generated.cpp for writing.', "\n";
		exit 1;
	}

	print F '/* This file is automatically generated by the configuration system', "\n";
	print F ' * Any changes will be lost when the configuration is saved as this', "\n";
	print F ' * file will be rewritten. */', "\n";

	print F '#include <mrd/mrd.h>', "\n\n";

	foreach (@mrd_modules) {
		if ($static_modules{$_}) {
			print F "extern \"C\" mrd_module *mrd_module_init_$_(void *, mrd *);\n";
		}
	}
	...
	foreach (@mrd_modules) {
		if ($static_modules{$_}) {
			print F "\tm_static_modules[\"$_\"] = &mrd_module_init_$_;\n";
		}
	}
	...
	print 'Wrote the static module list into src/modules.generated.cpp', "\n";
}
		...
		while (<F>) {
			if ($_ =~ m/^([A-Z_]+) = ([0-9A-Za-z_\$\(\)\/\. ]+)$/) {
				if ($1 eq 'SUPPORT_MODULES') {
					...
					$prefix = $2;
				} elsif ($1 eq 'STATIC_MODULE_SOURCES') {
					foreach (@mrd_modules) {
						$static_modules{$_} = 0;
					}
					foreach (split (/ /, $2)) {

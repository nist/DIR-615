...
base_stream &operator << (base_stream &os, const tval &t) {
	os << (uint32_t)t.v.tv_sec << ":" << (uint64_t)t.v.tv_usec;
	return os;
}
...
base_stream &operator << (base_stream &os, const timer_base &t) {
	os << t.name << ", ";

	if (t.is_running()) {
		os << "Running, " << timerdef::prettyprint(t.time_left());
	} else {
		os << "Stopped";
	}
...
void timermgr::handle_timer_event() {
	assert_debug(!tq.empty(), "empty timer list in timer event", return);
...
std::string timerdef::prettyprint(uint32_t interval) {
	char buf[64];

	_prettyprint(buf, sizeof(buf), interval);
...
bool timermgr::output_info(base_stream &ctx, bool extended) const {
	...
	char fmt[64];
	snprintf(fmt, sizeof(fmt), "| %%%is | %%12s | %%10s | %%8s |", namelen);

	_draw_sep(ctx, namelen);
	ctx.printf(fmt, "timer name", "time left", "interval", "repeat") << endl;
	_draw_sep(ctx, namelen);
	...
	for (tq_def::const_iterator i = tq.begin(); i != tq.end(); ++i) {
		...
		ctx.printf(fmt, h->name.c_str(), _prettyprint(buf1, sizeof(buf1), h->time_left()),
			_prettyprint(buf2, sizeof(buf2), h->_interval), h->_repeat ? "true" : "false") << endl;
	}

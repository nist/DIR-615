...
void interface::shutdown() {
	g_mrd->log().info(NORMAL) << "(MRD) removing " << *this << endl;
}
...
void interface::change_state(kernel_state newstate) {
	...
	loginfo(VERBOSE) << " state changed, "
		<< _kernel_state_names[mif_state] << " -> "
		<< _kernel_state_names[newstate] << endl;
...
bool interface::output_info(base_stream &_out, const std::vector<std::string> &args) const {
	...
	_out << "Interface " << name() << " (" << index() << ") is ";

	if (mif_enabled) {
		_out << (mif_state == Up ? "Up" : "Down");
	} else {
		_out << "Disabled";
	}

	_out << " (Uptime: " << timerdef::prettyprint(tval::now() - mif_creationtime) << ")" << endl;

	_out.inc_level();

	if (mif_linklocals.empty()) {
		_out << "Link-Local: (None)" << endl;
	} else {
		bool first = mif_globals.empty();
		for (std::set<inet6_addr>::const_iterator i = mif_linklocals.begin();
				i != mif_linklocals.end(); ++i) {
			_out << "Link-Local: " << *linklocal();
			if (first) {
				_out << " <PRIMARY>";
				first = false;
			}
...
base_stream &interface::loginfo(int level) const {
	return g_mrd->log().info(level) << "(" << mif_name << ") ";
}

base_stream &interface::logwarn() const {
	return g_mrd->log().warn() << "(" << mif_name << ") ";
}

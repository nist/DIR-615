...
bool bsd_rib::lookup_prefix(const in6_addr &dst, lookup_result &res) const {
	...
	g_mrd->log().info(NORMAL) << "(BSD-RIB) path towards dst " << dst << endl;
	...
	if (write(evsock.fd(), hdr, hdr->rtm_msglen) < 0) {
		g_mrd->log().warn() << "BSD-RIB: Failed to write " << hdr->rtm_msglen
			<< " bytes to route socket(" << evsock.fd() << "): " << strerror(errno) << endl;
		return 0;
	}
...
void bsd_rib::event_pending(rt_msghdr *rtm) {
	...
	g_mrd->log().info(NORMAL) << "BSD-RIB: Pending event type "
		<< _rtmsg_type_name(u.rt->rtm_type)
		<< " length " << (int)rtm->rtm_msglen << " bytes" << endl;

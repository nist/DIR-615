...
void _static_sources_node::group_interest_changed(group_node *n, bool include,
	...
	g_mrd->log().info(NORMAL) << "_static_sources_node::group_interest_changed entry\n";
	...
	g_mrd->log().info(NORMAL) << "_static_sources_node::group_interest_changed exit\n";
}

bool _static_sources_node::call_method(int id, base_stream &out,
				       const std::vector<std::string> &args) {
	g_mrd->log().info(NORMAL) << "_static_sources_node::call_method entry\n";
	switch (id) {
	...
	g_mrd->log().info(NORMAL) << "_static_sources_node::call_method return\n";
...
void source_discovery_origin::discovered_source(int ifindex,
	...
	g_mrd->log().info(NORMAL) << "source_discovery_origin::discovered_source entry\n";
	//g_mrd->log().info(NORMAL) << "g_dev" << g_dev << endl;
	g_mrd->discovered_source(ifindex, groupmask, source, this);
	g_mrd->log().info(NORMAL) << "source_discovery_origin::discovered_source exit\n";
}
...
void aggr_source_discovery::discovered_source(int ifindex,
	...
	g_mrd->log().info(NORMAL) << "aggr_source_discovery::discovered_source entry\n";
	if (add_to_cache(m_cache, ifindex, group, src) >= 0)
		source_discovery_origin::discovered_source(ifindex, group, src);
	g_mrd->log().info(NORMAL) << "aggr_source_discovery::discovered_source return\n";
}

int aggr_source_discovery::add_to_cache(cache &c, int ifindex,
					const inet6_addr &group,
					const inet6_addr &src) {
	g_mrd->log().info(NORMAL) << "aggr_source_discovery::add_to_cache entry\n";
	sg_pair p(group, src);

	cache::iterator i = m_cache.find(p);

	time_t now = time(0);

	if (i == m_cache.end()) {
		m_cache.insert(std::make_pair(p, now));

		g_mrd->log().info(NORMAL) << "AggrSourceDiscovery "
			<< " added source (" << src << ", "
			<< group << ") to cache" << endl;
	...
	g_mrd->log().info(NORMAL) << "aggr_source_discovery::add_to_cache return\n";
	return -1;
}
...
void static_source_discovery::groupconf_registered(groupconf *gc, bool include) {
	g_mrd->log().info(NORMAL) << "static: groupconf_registered " << gc->id() << " " << include << endl;
...
void source_discovery_sink::discovered_source(interface *input,
					      ...
					      source_discovery_origin *origin) {
	g_mrd->log().info(NORMAL) << "source_discovery_sink::discovered_source entry\n";
...
group_interface *group::local_oif(interface *intf) {
	...
	if (oif) {
		loginfo(VERBOSE) << "added " << *intf
				 << " to interface list" << endl;
...
void group::discovered_source(interface *input, const inet6_addr &source,
	...
	g_mrd->log().info(NORMAL) << "group::discovered_source entry\n";
	for (properties::const_iterator i = m_properties.begin();
			i != m_properties.end(); ++i) {
		if (is_group_node(i->second)) {
	...
	g_mrd->log().info(NORMAL) << "group::discovered_source return\n";
}
...
bool group::someone_lost_interest() {
	for (properties::const_iterator i = m_properties.begin();
		...
		if (grpnode->has_interest_in_group()) {
			g_mrd->log().info(NORMAL) << "not removing group as "
...
base_stream &operator << (base_stream &os, const group_interface &gif) {
	return os << *gif.g_owner << "!" << *gif.g_intf;
}

// stupid duplicate code

base_stream &group_interface::loginfo(int level) const {
	return g_mrd->log().info(level) << "(" << *this << ") ";
}

base_stream &group_interface::logwarn() const {
	return g_mrd->log().warn() << "(" << *this << ") ";
}

base_stream &group::loginfo(int level) const {
	return g_mrd->log().info(level) << "(" << *this << ") ";
}

base_stream &group::logwarn() const {
	return g_mrd->log().warn() << "(" << *this << ") ";
}

...
"$Id: plugin.c,v 1.3 2008/09/12 10:51:49 jimmy_huang Exp $";
...
/* jimmy added 20080912, for wantimer to check more precisely
...
*/
	...
	system("killall -SIGTTOU wantimer");//ppp auth failed or no discovery response
	error("Send SIGTTOU (pppoe discovery failed)to wantimer");
#endif	
/* ---------------------------------------------------------- */
	    return -1;
	}
    }
//Albert copy from rp-pppoe pppoe.c
//---------        
    /*NickChou, 07.11.06, add for sending PADT before PADI */
    {
	    FILE *fp;
		fp = fopen("/var/etc/pppoe_peer_mac", "w");
		if (fp) 
		{
			...
			syslog(LOG_INFO, "rp_pppoe => write /var/etc/pppoe_peer_mac fail");	
    }
//--------    
...
plugin_init(void)
{
	...
	*/
#ifdef DLINK_ROUTER_LED_DEFINE
	system("killall -SIGTTIN wantimer");
	info("RP-PPPoE plugin inform wantimer to reset auth, discovery variable");
#endif	
	/* ---------------------------------------------- */
}

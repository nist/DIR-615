...
"$Id: discovery.c,v 1.3 2008/12/03 11:58:04 albert Exp $";
...
//Albert copy from rp-pppoe discovery.c
//---------      
    /*NickChou, 07.11.06, add for sending PADT before PADI */
    {
    	FILE *fp;
		fp = fopen("/var/etc/pppoe_conn_session", "w");
		if(fp) 
		{
			...
			syslog(LOG_INFO, "rp_pppoe => write /var/etc/pppoe_conn_session error");
    }
//-----------    
...
//Albert added
//-----------
int transfer_mac(char *p, char *ea) 
{ 
	int i = 0;
...
//Albert copy from rp-pppoe discovery
//---------    
    {
    	FILE *fp1, *fp2;
    	fp1 = fopen("/var/etc/pppoe_conn_session", "r");
    	if(fp1)
    	{
			if(fgets(session_id, 20, fp1) == NULL)
				syslog(LOG_INFO, "rp_pppoe => get pppoe_conn_session error");
			else
				conn->session = (UINT16_t)htons(atoi(session_id));
				
			fp2 = fopen("/var/etc/pppoe_peer_mac", "r");
			if(fp2)
			{
				if(fgets(peer_mac, 20, fp2) == NULL)
					syslog(LOG_INFO, "rp_pppoe => get pppoe_peer_mac error");
				else	
					transfer_mac(peer_mac,conn->peerEth);
				syslog(LOG_INFO, "rp_pppoe => Send PADT to terminate the session ID that sent last time");	
				sendPADT(conn, "Send PADT to terminate the session ID that sent last time");
				fclose(fp2);
			}
			else
				syslog(LOG_INFO, "rp_pppoe => read pppoe_peer_mac error");

			fclose(fp1);
		}
		else
			syslog(LOG_INFO, "rp_pppoe => read pppoe_conn_session error or first time dial");	
    }
//----------        
    do {
	padiAttempts++;

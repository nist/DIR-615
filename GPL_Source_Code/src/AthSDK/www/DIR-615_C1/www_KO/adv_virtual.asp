					  			...
					  			if (port2 == "" || Aport2 == ""){	//when PF is sigle port or vs is sigle port		  	
							  		if(port1 == Aport1){
										...
										if(parseInt(port1) <= parseInt(Aport1) && parseInt(port2) >= parseInt(Aport1)){
									 	    ...
									 	    alert(obj_word + "어플리케이션 방화벽 포트와 충돌.");
									 	    return false;							
										}									
								  }
							}	
			...
			if (temp_port_name != ""){
				for (var j = i+1; j < rule_max_num; j++){
					if (temp_port_name == get_by_id("name" + j).value){
						...
						alert("Name"+ get_by_id("name" + j).value + "is already used.");	//av_alert_16
						return false;
					}
				}
			}
		...
		for (var i = 0; i < rule_max_num; i++){		
			...
			if (get_by_id("name" + i).value != ""){
				if(Find_word(check_name,"'") || Find_word(check_name,'"') || Find_word(check_name,"/")){
					...
					alert("포트 포워딩 이름 "+ i +"효력없는. 부적절한 문자 ',/,''");//TEXT002
					return false;
				}
				...
				if(protocol != 3){
					if(get_by_id("enable" + i).checked == true){
						if(!check_forward_port(protocol, get_by_id("public_portS" + i).value, which_lang[_vs_port])){
							return false;
						}
					
						if(!check_app_port(protocol,get_by_id("public_portS" + i).value, which_lang[_vs_port])){
							return false;
						}
						
						if(!check_virtual_port(i, get_by_id("public_portS" + i).value, get_by_id("ip" + i).value, which_lang[_vs_port])){
							return false;
						}
						if(!check_virtual_port(i, get_by_id("private_portS" + i).value, get_by_id("ip" + i).value, which_lang[_vs_port])){
							return false;
						}
					}
...
<body topmargin="1" leftmargin="0" rightmargin="0" bgcolor="#757575">
	...
	<table border="1" cellpadding="2" cellspacing="0" width="838" height="100%" align="center" bgcolor="#FFFFFF" bordercolordark="#FFFFFF">
		<tr>
                  ...
                  <tbody>
                    <tr>
                      <td id=sidenav_container>
                        <div id=sidenav>
                          <UL>
                            <LI><div id=sidenavoff><script>show_words(which_lang[_virtserv])</script></div></LI>
          ...
          <div id=box_header> 
            <h1>
              <script>show_words(which_lang[_vs_title])</script>
            </h1>
            <script>show_words(which_lang[av_intro_vs])</script>
            <br>
            <br>
            <input name="button" id="button" type="button" class=button_submit value="" onClick="send_request()">
			<input name="button2" id="button2" type="button" class=button_submit value="" onclick="page_cancel('form1', 'adv_virtual.asp');">
          ...
          <div class=box> 
            <h2>
              ...
              &ndash;&ndash;
              <script>show_words(which_lang[vs_vslist])</script>
            </h2>
                    <table borderColor=#ffffff cellSpacing=1 cellPadding=2 width=525 bgColor=#dfdfdf border=1>
                      <tbody>
                        <tr>
                          ...
                          <td width="31">&nbsp;</td>
                          <td width="195">&nbsp;</td>
                          <td width="130" align=middle><div align="left">
                      <script>show_words(which_lang[_vs_port])</script>
                    </div></td>
                          <td width="125" align=middle><div align="left">
                      <script>show_words(which_lang[_vs_traffictype])</script>
                    </div></td>
                        ...
                        <script>
							...
							for(var i=0 ; i<rule_max_num ; i++){
								...
								document.write('<td align=center valign=bottom>'+which_lang[av_PubP]+'<br><input type=text class=flatL id=public_portS'+ i + ' name=public_portS' + i +' size=5 maxlength=5></td>');
								...
								document.write("</select></td>");
								document.write('<td align=center valign=bottom>'+which_lang[av_PriP]+'<br><input type=text class=flatL id=private_portS' + i + ' name=private_portS' + i +' size=5 maxlength=5></td>');
      ...
      cellPadding=2 bgColor=#ffffff borderColorLight=#ffffff border=0>
            ...
            <td id=help_text><b><strong> 
              <script>show_words(which_lang[_hints])</script>
                ...
                <script>show_words(which_lang[hhav_name])</script>
              </p>
                        
              <p>
                <script>show_words(which_lang[hhav_ip])</script>
              </p>
                        
              <p>
                <script>show_words(which_lang[hhav_sch])</script>
              </p>
                        
              <p>
                <script>show_words(which_lang[hhav_filt])</script>
              </p>
						<p><a href="support_adv.asp#Virtual_Server" onclick="return jump_if();"><script>show_words(which_lang[_more])</script>&hellip;</a></p>
					   </td>
                    </tr>
                  </tbody>
			  </table></td>

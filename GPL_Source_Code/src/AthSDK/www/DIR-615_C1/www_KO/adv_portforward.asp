					  	...
					  	if(potocol == temp_vs[2] || temp_vs[2]==256){
					  			if (port2 == ""){			  	
							  		if(port1 == temp_vs[3]){
							  				...
							  				alert(obj_word + "버츄얼 서버 포트와 충돌.");//TEXT056
												return false;	
										}
									}
									else{//range
			...
			if (temp_port_name != ""){
				for (var j = i+1; j < rule_max_num; j++){
					if (temp_port_name == get_by_id("name" + j).value){
						alert("포트 포워딩 이름은 이미 리스트에 있습니다.");//TEXT060
		...
		for (var i = 0; i < rule_max_num; i++){		
			...
			if (get_by_id("name" + i).value != ""){
					...
					if (get_by_id("tcp_ports" + i).value == "0" && get_by_id("udp_ports" + i).value == "0"){
						alert("TCP 포트 이름 또는 UDP 포트 이름을 입력해 주십시요.");//TEXT061
					...
					if(get_by_id("tcp_ports" + i).value != ""){
							...
							if(!check_multi_port(remote_port, get_by_id("tcp_ports" + i).value)){
								...
								alert("TCP ports[" + get_by_id("tcp_ports" + i).value+ "] conflicts with Remote Management Port");	//ag_conflict4
								return false;
							}
								}
							}

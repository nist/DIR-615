	...
	for(var i = 1; i < 5; i++){
			...
			if (wep_def_key[i-1].selectedIndex){
		    	...
		    	if (key.length != wep_key_len && key.length != hex_len){
			    		//alert("The Key " + i + " is invalid. The Key must be " + wep_key_len + " characters or " + hex_len + " hexadecimal number.");//TEXT041
			    		alert("키값 " + i + " 부적절함. 키값는 반드시 " + wep_key_len + " 문자 또는 " + hex_len + " hexadecimal number.");	//TEXT041
			    		return false;
		    	}else if(key.length == hex_len){
			      	for (var j = 0; j < key.length; j++){
			      		if (!check_hex(key.substring(j, j+1))){
			      			//alert("Key " + i + " is wrong, the legal characters are 0~9, A~F, or a~f.");//TEXT042
			      			alert("키값  "+ i + " 는 틀림, 유효한 문자는 0~9, A~F, or a~f.");
			      			return false;
			      		}
			      	}
			      	if(i == def_key)

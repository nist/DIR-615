...
function save_reserved(){
	...
	if(reserved_name == ""){
		...
		alert("보유 IP 주소 " + reserved_ip + " 는 설정되어 있는 DHCP 범위 내에 있어야 합니다.");//GW_DHCP_SERVER_RESERVED_IP_IN_POOL_INVALID
		return false;
        ...
        if (check_resip_order(temp_res_ip_obj,start_obj, end_obj)){
			...
			alert("보유 IP 주소 " + reserved_ip + " 는 설정되어 있는 DHCP 범위 내에 있어야 합니다.");
			return false;
		}
		...
		for(m = 1; m < DataArray.length; m++){
				...
				if(reserved_mac.length > 0){
					if((reserved_mac == DataArray[m].MAC)){
						...
						alert("이 MAC 주소 "+ reserved_mac +" 의 보유 IP 주소는 이미 사용중입니다.");
						return false;
					}
				}
			}	

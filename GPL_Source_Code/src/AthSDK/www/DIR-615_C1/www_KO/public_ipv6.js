	 ...
	 if(_index<=-1)			
		...
		if((_index-current_index)==1){
			...
			if(dc_flag>1){
				...
				alert("IPv6 주소는 더블-콜론이 단 한번 허용됩니다.");
				return 2;
			}
		}	
...
function check_ipv6_address(my_obj,strFind){
	...
	if(strFind == "::"){ 		
		if (my_obj.addr.length == 2){ //檢查是否有2個區塊
			...
			if(sum == 0){   // 0 represents ipv6 has zero address
	...
	if( strOrg.length > 0 && strOrg.length < 5){
		for(var index =0; index < strOrg.length ;index++){
			if(!check_hex(strOrg.charAt(index))){
				...
				alert("서브넷 "+tag +" 는 반드시 16진수 여야 합니다.");
				return false;
			}	 	
		}
	}else{
		//alert("The suffix of "+tag+" is not an invalid address.");
		alert("서브넷 "+tag+" 는 적절하지 않은 주소입니다.");
		return false;	
	}

...
function save_reserved(){
	...
	if(reserved_name == ""){
		...
		alert("保留的IP地址 " + reserved_ip + " 应该在所配置的DHCP范围内。");//GW_DHCP_SERVER_RESERVED_IP_IN_POOL_INVALID
		return false;
        ...
        if (check_resip_order(temp_res_ip_obj,start_obj, end_obj)){
			...
			alert("保留的IP地址 " + reserved_ip + " 应该在所配置的DHCP范围内。");
			return false;
		}
		...
		for(m = 1; m < DataArray.length; m++){
				...
				if(reserved_mac.length > 0){
					if((reserved_mac == DataArray[m].MAC)){
						...
						alert("为该MAC地址 "+ reserved_mac +" 保留的 IP地址已经设置。");

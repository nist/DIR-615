		...
		void *userinfo)
{
	...
	if (info->mode == MODE_DNAT)
		return porttrigger_nat(pskb, in, out, hooknum, targinfo, userinfo);
	else if (info->mode == MODE_FORWARD_OUT)
		return porttrigger_forward(pskb, hooknum, in, out, targinfo, userinfo, MODE_FORWARD_OUT);
	else if (info->mode == MODE_FORWARD_IN)
		return porttrigger_forward(pskb, hooknum, in, out, targinfo, userinfo, MODE_FORWARD_IN);
...
static struct ipt_target porttrigger = {
	.name		= "PORTTRIGGER",
...
{
	return ipt_register_target(&porttrigger);
}

static void __exit fini(void)
{
	ipt_unregister_target(&porttrigger);
}

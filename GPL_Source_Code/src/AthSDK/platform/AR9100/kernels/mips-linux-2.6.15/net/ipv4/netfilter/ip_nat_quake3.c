...
quake3_nat_help(struct ip_conntrack_expect *exp)
{
	...
	DEBUGP("quake3_nat_help: start\n");
		
	if(ct==NULL)
	{
                /*NickChou add, 20090522
                FPS games (Nexuiz) cause router to reboot when attempting to contact online servers.
                */		
                DEBUGP("quake3_nat_help: exp->master NULL\n");
		return NF_ACCEPT;
	}		

			 ...
			 struct in6_addr *target)
{
	...
	if (ipv6_get_lladdr(dev, &saddr_buf)) {
        ...
        printk("ip dest addr=%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", NIP6(skb->nh.ipv6h->daddr));
        printk("ip source addr=%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", NIP6(skb->nh.ipv6h->saddr));
    ...
    /*CHT IOT test : filter redundant redirect packet*/
    if(skb->nh.ipv6h->daddr.s6_addr32[0]== skb->nh.ipv6h->saddr.s6_addr32[0] &&
       skb->nh.ipv6h->daddr.s6_addr32[1]== skb->nh.ipv6h->saddr.s6_addr32[1] )
    {
        printk("source and dest prefix are the same\n");
        return;
    }

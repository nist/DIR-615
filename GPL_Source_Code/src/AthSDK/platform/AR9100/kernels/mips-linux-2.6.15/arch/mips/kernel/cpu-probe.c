...
__init void cpu_probe(void)
{
	...
	case PRID_COMP_LEGACY://0x000000
		printk("PRID_COMP_LEGACY CPU\n");
		cpu_probe_legacy(c);
		break;
	case PRID_COMP_MIPS:
		printk("PRID_COMP_MIPS CPU\n");
		cpu_probe_mips(c);
		break;
	case PRID_COMP_ALCHEMY:
		printk("PRID_COMP_ALCHEMY CPU\n");
		cpu_probe_alchemy(c);
		break;
	case PRID_COMP_SIBYTE:
		printk("PRID_COMP_SIBYTE CPU\n");
		cpu_probe_sibyte(c);
		break;
	case PRID_COMP_SANDCRAFT:
		printk("PRID_COMP_SANDCRAFT CPU\n");
		cpu_probe_sandcraft(c);
		break;
 	case PRID_COMP_PHILIPS:
 		printk("PRID_COMP_PHILIPS CPU\n");
		cpu_probe_philips(c);
 		break;
	default:
		printk("CPU_UNKNOWN\n");
...
__init void cpu_report(void)
{
	...
	printk("cpu_report:CPU revision is: %08x\n", c->processor_id);
	if (c->options & MIPS_CPU_FPU)
